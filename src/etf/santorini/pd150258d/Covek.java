...
public class Covek extends Igrac {
	...
	public void prviDeoPoteza() throws InterruptedException, Greska {
		...
		while (true) {
			...
			b = true;
			try {
				a = !tabla.oznaka(izabranaFig.getRed(), izabranaFig.getKolona()).substring(0, 4)
						.equals(figure[0].getOznaka().substring(0, 4));
				b = !tabla.oznaka(izabranaFig.getRed(), izabranaFig.getKolona()).substring(0, 4)
						.equals(figure[1].getOznaka().substring(0, 4));
			} catch (Exception e) {
			}
			if (!a || !b)
				break;
		...
		while (dalekaPozicija(novaPozicija, izabranaFig) || !dozvoljena(novaPozicija)) {

			a = true;
			b = true;
			try {
				a = !tabla.oznaka(novaPozicija.getRed(), novaPozicija.getKolona()).substring(0, 4)
						.equals(figure[0].getOznaka().substring(0, 4));
				b = !tabla.oznaka(novaPozicija.getRed(), novaPozicija.getKolona()).substring(0, 4)
						.equals(figure[1].getOznaka().substring(0, 4));
			} catch (Exception e) {
			}
		...
		tabla.postaviOznaku(izabranaFig.getRed(), izabranaFig.getKolona(),
	...
	public void drugiDeoPoteza() throws InterruptedException, Greska {
		...
		while (dalekaPozicija(izabranoPolje, izabranaFigura.getKoord()) || !dozvoljenoZaGradnju(izabranoPolje)) {
